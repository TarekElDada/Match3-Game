...
export default class Cookie {
  ...
  static swapCookies(c1, c2, grille) {
    ...
    if (Cookie.distance(c1, c2) === 1) {
      ...
      //on joue le son
      //let audio = new Audio("..\\assets\\sons\\swap.wav");
      //audio.play();

      let grille = new Grille();
      grille.jouerSon("swap"); // Joue le son de swap

    }

    // on les désélectionne
    c1.deselectionnee();
    c2.deselectionnee();

    // on teste si le swap a créé un alignement
    /*grille.testAlignementDansTouteLaGrille();

    setTimeout(() => {
      // Chute des colonnes
      for (let c = 0; c < this.colonnes; c++) {
        grille.chuteColonne(c);
      }
    }, 400);*/
